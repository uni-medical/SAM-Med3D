...
from utils.data_loader import SegFM3D_Dataset, Union_Dataloader
...
class RandomCrop:
    ...
    def __call__(self, sample):
        ...
        spatial_shape = torch.Tensor([sample.spatial_shape]).squeeze()
...
def get_dataloaders(args):
    ...
    resize_transform = tio.Resize(
        target_shape=(args.img_size, args.img_size, args.img_size)
    )
    choose_one_transform = tio.OneOf(
        {
            crop_transform: 0.5,
            resize_transform: 0.5,
        }
    )

    train_dataset = SegFM3D_Dataset(paths=img_datas, transform=tio.Compose([
        tio.ToCanonical(),
        choose_one_transform,
...
class BaseTrainer:
    ...
    def set_optimizer(self):
        ...
        self.optimizer = torch.optim.AdamW([
            {'params': sam_model.image_encoder.parameters(), 'lr': self.args.lr},
    ...
    def train_epoch(self, epoch, num_clicks):
        ...
        for step, data3D in enumerate(tbar):
            try:
                ...
                logger.info(f"Error processing batch at step {step}: {e}")
            ...
            if not self.args.multi_gpu or (self.args.multi_gpu and self.args.rank == 0):
                if step % self.args.accumulation_steps == 0 and step != 0:
                    ...
                    logger.info(f'Epoch: {epoch}, Step: {step}, Loss: {print_loss}, Dice: {print_dice}')
