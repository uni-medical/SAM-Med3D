    ...
    with torch.no_grad():
        ...
        if (roi_gt is not None):
            prev_low_res_mask = prev_low_res_mask if (
                prev_low_res_mask is not None) else torch.zeros(1, 1, roi_image.shape[2] //
                                                                4, roi_image.shape[3] //
...
def data_preprocess(img_path, gt_path, category_index, target_spacing=[1.5, 1.5, 1.5]):
    ...
    subject, meta_info = read_and_resample_nifti(
        full_img,
        cls_gt,
        meta_info,
...
def data_postprocess(roi_pred, meta_info):
    ...
    pred3D_full_ori = F.interpolate(torch.Tensor(pred3D_full)[None][None],
...
def validate_paired_img_gt(model, img_path, gt_path, output_path):
    ...
    for category_index in exist_categories:
        roi_image, roi_label, meta_info = data_preprocess(img_path,
                                                          gt_path,
