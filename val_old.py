...
parser.add_argument('-tdp', '--test_data_path', type=str, default='./data/resampled_test_val_set')
parser.add_argument('-vp', '--vis_path', type=str, default='./data/vis')
parser.add_argument('-cp', '--checkpoint_path', type=str, default='./ckpt/sam_med3d_turbo.pth')
parser.add_argument('--save_name', type=str, default='./data/resampled_test_val_set/dice.py')
...
if __name__ == "__main__":    
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        image3D, gt3D, img_name = batch_data['image'].to(device), batch_data['label'].to(device), batch_data['path']
        sz = image3D.size()
        ...
        if(args.skip_existing_pred and os.path.exists(pred_path)):
            ...
            if(args.dim==3):
                seg_mask_list, points, labels, iou_list, dice_list = finetune_model_predict3D(
                    ...
                    click_method=args.point_method, num_clicks=args.num_clicks, l
                    prev_masks=None)
            elif(args.dim==2):
    ...
    args.save_name = args.save_name.replace('.py', f'_nc{args.num_clicks}.py')
    if(args.split_num>1):
